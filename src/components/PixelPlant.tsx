...
const LEAVES: Record<'left' | 'center' | 'right', LeafConfig> = {
  left: { origin: '192,283', fromX: -3, toX: 4, fromY: -1, toY: 1, duration: 1800, delay: 0 },
  center: { origin: '251.5,283', fromX: 1, toX: -1, fromY: -2, toY: 0, duration: 2100, delay: 600 },
  right: { origin: '312.5,283', fromX: -3, toX: 4, fromY: 0, toY: -2, duration: 1950, delay: 1400 },
};

function useSway(from: number, to: number, duration: number, delay: number) {
...
export default function PixelPlant({ size = 160, style }: PixelPlantProps) {
  const leftTX = useSway(LEAVES.left.fromX, LEAVES.left.toX, LEAVES.left.duration, LEAVES.left.delay);
  const leftTY = useSway(LEAVES.left.fromY, LEAVES.left.toY, LEAVES.left.duration, LEAVES.left.delay);
  const centerTX = useSway(LEAVES.center.fromX, LEAVES.center.toX, LEAVES.center.duration, LEAVES.center.delay);
  const centerTY = useSway(LEAVES.center.fromY, LEAVES.center.toY, LEAVES.center.duration, LEAVES.center.delay);
  const rightTX = useSway(LEAVES.right.fromX, LEAVES.right.toX, LEAVES.right.duration, LEAVES.right.delay);
  const rightTY = useSway(LEAVES.right.fromY, LEAVES.right.toY, LEAVES.right.duration, LEAVES.right.delay);
  ...
  return (
    <View style={[styles.container, { width: size, height: size }, style]}>
      <Svg width="100%" height="100%" viewBox="0 0 500 500">
        ...
        <AnimatedG origin={LEAVES.left.origin} style={{ transform: [{ translateX: leftTX }, { translateY: leftTY }] }}>
          <Path
        ...
        <AnimatedG origin={LEAVES.center.origin} style={{ transform: [{ translateX: centerTX }, { translateY: centerTY }] }}>
          <Path
        ...
        <AnimatedG origin={LEAVES.right.origin} style={{ transform: [{ translateX: rightTX }, { translateY: rightTY }] }}>
          <Path
